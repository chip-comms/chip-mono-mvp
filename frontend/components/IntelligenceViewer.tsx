...
import { Intelligence, ActionItem, CompanyValue, SpeakerStats } from '@/lib/types';
...
export default function IntelligenceViewer({ intelligence, recordingTitle }: IntelligenceViewerProps) {
    ...
    <div className="space-y-6">
      ...
      {intelligence.insights && (
        <div className="bg-purple-50 border border-purple-200 rounded-lg p-6">
          ...
          <p className="text-gray-700 leading-relaxed">{intelligence.insights}</p>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white border rounded-lg p-4 text-center">
          <div className="text-2xl font-bold text-blue-600">{intelligence.actionItems.length}</div>
          <div className="text-sm text-gray-600">Action Items</div>
        </div>
        <div className="bg-white border rounded-lg p-4 text-center">
          <div className="text-2xl font-bold text-green-600">{intelligence.keyTopics.length}</div>
          <div className="text-sm text-gray-600">Topics</div>
        </div>
        <div className="bg-white border rounded-lg p-4 text-center">
          <div className="text-2xl font-bold text-purple-600">{intelligence.speakerStats.length}</div>
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex space-x-2">
          <button
            onClick={() => copyToClipboard(intelligence.transcript, 'Transcript')}
  ...
  const renderActionsTab = () => (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="text-sm text-gray-600">{intelligence.actionItems.length} items</div>
      </div>
  ...
  const renderMetricsTab = () => (
    <div className="space-y-6">
      ...
      <div className="bg-white border rounded-lg p-6">
        <h4 className="text-lg font-semibold text-gray-900 mb-4">Speaking Time Distribution</h4>
        <div className="space-y-3">
          {intelligence.speakerStats.map((speaker: SpeakerStats, index: number) => (
            <div key={index} className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-4">
                <div className="text-sm text-gray-600">
                  {Math.round(speaker.duration / 60)}m {speaker.duration % 60}s
                </div>
                ...
                <div className="text-sm font-medium w-12 text-right">{speaker.percentage}%</div>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* Response Delays */}
      <div className="bg-white border rounded-lg p-6">
        <h4 className="text-lg font-semibold text-gray-900 mb-4">Response Patterns</h4>
        {intelligence.communicationMetrics.responseDelays.length === 0 ? (
          <p className="text-gray-500">No response delay data available</p>
        ) : (
          <div className="space-y-3">
            {intelligence.communicationMetrics.responseDelays.map((delay, index) => (
              <div key={index} className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                ...
                <div className="flex items-center space-x-2">
                  <span className={`text-sm px-2 py-1 rounded ${
                    delay.delaySeconds < 0 ? 'bg-red-100 text-red-700' :
                    delay.delaySeconds < 1 ? 'bg-green-100 text-green-700' :
                    delay.delaySeconds < 3 ? 'bg-yellow-100 text-yellow-700' :
                    'bg-gray-100 text-gray-700'
                  }`}>
                    {delay.delaySeconds < 0 ? 'Interrupted' : `${delay.delaySeconds.toFixed(1)}s delay`}
                  </span>
                </div>
              </div>
            ))}
      ...
      {/* Company Values */}
      <div className="bg-white border rounded-lg p-6">
        <h4 className="text-lg font-semibold text-gray-900 mb-4">Company Values Alignment</h4>
        <div className="space-y-4">
          {intelligence.communicationMetrics.companyValuesAlignment.values.map((value: CompanyValue, index: number) => (
            <div key={index} className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-medium">{Math.round(value.score * 100)}%</span>
              </div>
              <div className="w-full bg-gray-200 rounded-full h-2">
                <div
                  className={`h-2 rounded-full ${
                    value.score >= 0.8 ? 'bg-green-600' :
                    value.score >= 0.6 ? 'bg-yellow-600' :
                    'bg-red-600'
                  }`}
              ...
              {value.examples.length > 0 && (
                <div className="pl-7">
                  <div className="text-xs text-gray-600 font-medium mb-1">Examples:</div>
                  <ul className="text-xs text-gray-600 space-y-1">
                    {value.examples.slice(0, 2).map((example, i) => (
                      <li key={i} className="before:content-['•'] before:mr-2">&quot;{example}&quot;</li>
                    ))}
                  </ul>
                </div>
              )}
        ...
        {intelligence.communicationMetrics.companyValuesAlignment.insights && (
          <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded">
            <p className="text-sm text-blue-800">
              <strong>Insight:</strong> {intelligence.communicationMetrics.companyValuesAlignment.insights}
            </p>
          </div>
        )}
      </div>
  ...
  const renderTopicsTab = () => (
    <div className="space-y-6">
      {/* Sentiment */}
      <div className="bg-white border rounded-lg p-6">
        <h4 className="text-lg font-semibold text-gray-900 mb-4">Overall Sentiment</h4>
        <div className="flex items-center space-x-4">
          <div className={`px-4 py-2 rounded-full text-sm font-medium ${
            intelligence.sentiment.overall === 'positive' ? 'bg-green-100 text-green-700' :
            intelligence.sentiment.overall === 'negative' ? 'bg-red-100 text-red-700' :
            'bg-gray-100 text-gray-700'
          }`}>
          ...
          <div className="flex-1 bg-gray-200 rounded-full h-2">
            <div
              className={`h-2 rounded-full ${
                intelligence.sentiment.overall === 'positive' ? 'bg-green-600' :
                intelligence.sentiment.overall === 'negative' ? 'bg-red-600' :
                'bg-gray-600'
              }`}
              style={{ width: `${Math.abs(intelligence.sentiment.score) * 100}%` }}
            ></div>
          </div>
      ...
      {/* Key Topics */}
      <div className="bg-white border rounded-lg p-6">
        <h4 className="text-lg font-semibold text-gray-900 mb-4">Key Topics Discussed</h4>
